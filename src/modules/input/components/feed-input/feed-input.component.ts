...
export class FeedInputComponent {
  ...
  onMessageChanged(message: string) {
    ...
    const delimitor: string = '@'
    const re = /@([^ ]*)$/
    if (this.message.includes(delimitor) ){
      const lastDelimitor = this.message.lastIndexOf(delimitor)
      let mention: string = this.message.substring(lastDelimitor +1);
      if(!mention.includes(' ')) {
        this.searchMentionedUsers(mention);
        this.showMentionList(this.message.match(re)!)
      }
  ...
  */
  ...
  send() {
    ...
    let messagePayload: MessageSentEventPayload = {
      date: dateTime,
